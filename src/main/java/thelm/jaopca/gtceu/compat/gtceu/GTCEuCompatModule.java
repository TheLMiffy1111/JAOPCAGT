...
public class GTCEuCompatModule implements IModule {
	...
	public void onCommonSetup(IModuleData moduleData, FMLCommonSetupEvent event) {
		...
		ResourceLocation darkAshDustLocation = ResourceLocation.parse("c:dusts/dark_ash");
		ResourceLocation fileLocation = ResourceLocation.parse("c:tools/file");
		ResourceLocation hardHammerLocation = ResourceLocation.parse("c:tools/hammer");
		ResourceLocation wrenchLocation = ResourceLocation.parse("c:tools/wrench");
		for(IMaterial material : moduleData.getMaterials()) {
			...
			if(type.isCrystalline() && !BLACKLIST.contains(name) && !configImplosionToCrystalBlacklist.contains(name)) {
				...
				if(itemTags.contains(dustLocation)) {
					helper.registerGTRecipe(
							miscHelper.getRecipeKey("gtceu.dust_to_material_implosion_powderbarrel", name),
					...
					helper.registerGTRecipe(
							...
							itemOutput(darkAshDustLocation, 1, 2500, 0));
					helper.registerGTRecipe(
							miscHelper.getRecipeKey("gtceu.dust_to_material_implosion_dynamite", name),
							GTRecipeTypes.IMPLOSION_RECIPES,
							helper.recipeSettings().
							itemInput(dustLocation, 4).
							...
							itemOutput(darkAshDustLocation, 1, 2500, 0));
					helper.registerGTRecipe(
							miscHelper.getRecipeKey("gtceu.dust_to_material_implosion_itnt", name),
							GTRecipeTypes.IMPLOSION_RECIPES,
							helper.recipeSettings().
							itemInput(dustLocation, 4).
